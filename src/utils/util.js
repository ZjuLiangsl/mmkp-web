...
export function welcome() {
  const arr = ['Let\'s take a break', 'Let\'s take a break?', 'Let\'s take a break', 'Let\'s take a break']
...
export function jsExpand(options = {}) {
  ...
  window[windowKeyName]['EVENT_' + id] = {
    ...
    // 当执行完成时，无论如何都会调用的回调事件
      ...
      // 执行完后 Delete 新增的 script 标签不会撤销执行结果（已产生的结果不会被撤销）
      script.outerHTML = ''
...
/**
 * 根据编码Constraint Strategycode，校验传入的值是否合法
 *
 * 使用示例：
 * { validator: (rule, value, callback) => validateCheckRule('common', value, callback) }
 *
 * @param ruleCode 编码Constraint Strategy code
 * @param value 被验证的值
 * @param callback
 */
