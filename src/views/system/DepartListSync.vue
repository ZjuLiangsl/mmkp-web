    ...
    <a-col :md="12" :sm="24">
      <a-card :bordered="false">
        ...
        <a-row style="margin: 0 0 0 14px" class="table-operator">
          ...
          <a-button type="primary" icon="download" @click="handleExportXls('部门信息')">Export</a-button>
          <a-upload name="file" :showUploadList="false" :multiple="false" :headers="tokenHeader"
                    :action="importExcelUrl" @change="handleImportExcel">
            <a-button type="primary" icon="import">Import</a-button>
          </a-upload>
          <j-third-app-button biz-type="depart" :selected-row-keys="selectedRowKeys" syncToApp
                              @sync-finally="onSyncFinally"/>
          <a-button title=" Delete 多条数据" @click="batchDel" type="default">Batch Delete </a-button>
        </a-row>
        ...
        <a-tab-pane tab="基本信息" key="1">
          <a-card :bordered="false" v-if="selectedKeys.length>0">
            ...
            <div class="anty-form-btn">
              <a-button @click="emptyCurrForm" type="default" htmlType="button" icon="sync">Reset</a-button>
  ...
  methods: {
    ...
    batchDel: function() {
      ...
      if (this.checkedKeys.length <= 0) {
        ...
        this.$confirm({
          title: 'Delete ',
          content: 'Sure you want to Delete 所选中的 ' + this.checkedKeys.length + ' 条数据，以及子节点数据吗?',
          onOk: function() {
    ...
    handleDelete() {
      ...
      this.$confirm({
        title: 'Delete ',
        content: 'Sure you want to Delete 此部门以及子节点数据吗?',
        onOk: function() {
          deleteByDepartId({ id: that.rightClickSelectedKey }).then((resp) => {
            if (resp.success) {
              // Delete the success后，去除已选中中的数据
              that.checkedKeys.splice(that.checkedKeys.findIndex(key => key === that.rightClickSelectedKey), 1)
              that.$message.success(' Delete the success!')
              that.loadTree()
              // Delete 后同步清空右侧基本信息内容
              let orgCode = that.model.orgCode
              if (orgCode && orgCode === that.rightClickSelectedOrgCode) {
                that.onClearSelected()
              }
            } else {
              that.$message.warning(' Delete  failure!')
            }
          })
        }
      })
