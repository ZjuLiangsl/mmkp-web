  ...
  >
    <a-alert type="info" showIcon style="margin-bottom: 16px;">
      <template slot="message">
        ...
        <template v-if="selectedRowKeys.length>0">
          ...
          <a-divider type="vertical"/>
          <a @click="handleDeleteBatch">批量Delete</a>
        </template>
      </template>
    </a-alert>
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        ...
        <a-divider type="vertical"/>
        <a @click="handleDelete([record.id])"><a-icon type="delete"/> 彻底Delete</a>
      </span>
    </a-table>
...
function superRequest(options) {
  ...
  options.promise.then(res => {
    ...
    console.error('查询已 Delete 的用户失败：', e)
    this.$message.warning('查询已 Delete 的用户失败：' + (e.message || e))
  }).finally(() => {
    this.loading = false
  })
}
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        ...
        // 回收站操作，get = 获取列表；put = 取回；delete = 彻底 Delete
  ...
  methods: {
    ...
    handleRevert(userIds) {
      this.$confirm({
        title: '恢复用户',
        content: `您Sure you want to恢复这 ${userIds.length} 个用户?`,
        centered: true,
        onOk: () => {
        ...
        content: (< div>
          < p>您Sure you want to Delete 这
            ...
            <
              /p>
              < p
                style="color:red;">注意： Delete 后将无法恢复，请谨慎操作！
                <
                  /p>
                  < /div>),
                    centered: true,
                    ...
                    () => {
                      ...
                      this.loadData()
                      this.handleClearSelection()
                      this.$message.success(` Delete  ${userIds.length} 个用户成功！`)
                    } else {
