      ...
      <a-card :bordered="false">
        ...
        <div>
          <a-table
            ...
            :loading="loading"
            :rowSelection="{selectedRowKeys: fkSelectedRowKeys, onChange: onFkSelectChange}"
            @change="handleTableChange">
          <span slot="action" slot-scope="text, record">
            <a @click="handleFkEdit(record)">Edit</a>
...
export default {
  ...
  components: {
    'fk-data-query-edit-modal': () => import('./DataQueryEditModal.vue')
  },
  data() {
    return {
      title: '选择',
  ...
  methods: {
    ...
    handleOk() {
      if (this.selectedRow && this.selectedRow[this.fkFieldName]) {
        this.$emit('ok', this.fieldName, this.selectedRow[this.fkFieldName])
      } else {
        this.$emit('ok', this.fieldName, '')
      }
      this.close()
    },
    handleCancel() {
    ...
    loadMeta(title, dsCode, dbName, tableName, fkFieldName, fieldName) {
      this.table = {}
      this.pkFields = []
      this.searchFields = []
      this.formFields = []
      this.columns = []
      this.fkSelectedRowKeys = []
      this.selectedRow = {}
      ...
      getAction('/md/tables/table-fields-by-params', {
        ...
        if (res.success == true) {
          ...
          if (this.table['isPaging'] == '1') {
            ...
            this.url.list = 'dqe/page?dsCode=' + dsCode + '&dbName=' + dbName + '&tableName=' + tableName
          } else {
            this.isPagination = false
            this.url.list = 'dqe/list?dsCode=' + dsCode + '&dbName=' + dbName + '&tableName=' + tableName
          }
          this.loadData()
    ...
    onFkSelectChange(selectedRowKeys, selectedRows) {
      this.fkSelectedRowKeys = [selectedRowKeys[selectedRowKeys.length - 1]]
      this.selectedRow = selectedRows[selectedRows.length - 1]
    },

    handleFkAdd() {
      console.log(this.$refs)
      this.$refs.modalFormFk.add()
      this.$refs.modalFormFk.title = 'New'
      this.$refs.modalFormFk.disableSubmit = false
    },
    handleFkEdit(record) {
      console.log(this.$refs)
      this.$refs.modalFormFk.edit(record)
      this.$refs.modalFormFk.title = 'Edit'
      this.$refs.modalFormFk.disableSubmit = false
    },
