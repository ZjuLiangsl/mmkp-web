<template>
  <div>
    <div class="left_div">
      <div id="meditor_model_div_id" class="meta_model_div">
        <VisualModeling
          ref="visualModel"
          :data="modelData"
...
export default {
  ...
  methods: {
              ...
              this.$nextTick(() => {
                ...
                postAction('/md/tables/saveEdge', edge).then(res => {
                  if (res.success == true) {
                    ...
                    this.$nextTick(() => {
                      this.$nextTick(() => {
                        this.$message.success('Incidence relation Success！')
                      })
                    })
                  } else {
    ...
    setStyle() {
      let doc = document.getElementsByClassName('meta_model_div')[0]
    ...
    sync() {
      ...
      if (this.currentNode.options) {
        ...
        const that = this
        this.$confirm({
          title: 'Whether all Category in the canvas are synchronized?',
          content: 'After a node is selected, only the Category related to the node are synchronized',
          onOk() {
            let ids = ''
