...
| actionButton | boolean |      | 是否显示操作按钮，包括"新增"、" Delete "，默认false                                 |
...
|-----------------|----------------------------------------------------|--------------------------------------------------|
| added           | 当添加行操作完成后触发                             |                                                  |
| deleted         | 当 Delete 行操作完成后触发（Batch Delete 操作只会触发一次） | `deleteIds` 被逻辑 Delete 的id                       |
...
| 参数名 | 类型            | 必填 | 说明                                                                                       |
|--------|-----------------|------|--------------------------------------------------------------------------------------------|
| id     | string 或 array | ✔️    | 被 Delete 行的id。如果要 Delete 一个，可以直接传id，如果要 Delete 多个，需要将多个id封装成一个数组传入 |
...
用于获取被逻辑 Delete 的行的id，返回一个数组，用户可将该数组传入后台，并进行Batch Delete 
