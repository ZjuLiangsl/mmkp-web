      ...
      <a-col>
        ...
        <div v-if="actionButton" class="action-button">
          ...
          <template v-if="selectedRowIds.length>0">
            <a-popconfirm
              :title="`Sure you want to Delete 这 ${selectedRowIds.length} 项吗?`"
              @confirm="handleConfirmDelete">
              <a-button v-if="buttonPermission('batch_delete')" type="primary" icon="minus" :disabled="disabled"> Delete </a-button>
              <span class="gap"></span>
            </a-popconfirm>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      initialize() {
        this.visibleTrEls = []
        // 判断是否是首次进入该方法，如果是就不清空行，防止 Delete 了预添加的数据
        if (!this.isFirst) {
         this.clearRow();
        } else {
          this.isFirst = false
      ...
      removeRows(id) {
        ...
        ids.forEach(removeId => {
          ...
          // 找到rows对应的index，并 Delete
          if (findAndDelete(rows)) {
            // 找到values对应的index，并 Delete
      ...
      buildTdStyle(col) {
        ...
        //update-begin-author:lvdandan date:20201116 for:LOWCOD-984 默认风格Function Test附表样式问题 日期时间控件长度太大
        //如果是时间控件设为200px
        ...
        //update-end-author:lvdandan date:20201116 for:LOWCOD-984 默认风格Function Test附表样式问题 日期时间控件长度太大
