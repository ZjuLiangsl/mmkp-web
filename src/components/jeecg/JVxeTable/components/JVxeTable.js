...
export default {
  ...
  props: {
    ...
    toolbarConfig: PropTypes.object.def(() => ({
      // prefix 前缀；suffix 后缀；
      slots: ['prefix', 'suffix'],
      // add 新增按钮；remove  Delete 按钮；clearSelection 清空选择按钮；collapse 展开收起
    ...
    // 是否异步 Delete 行，如果你要实现异步 Delete ，那么需要把这个选项开启，
    // 在remove事件里调用confirmRemove方法才会真正 Delete （除非 Delete 的全是新增的行）
  ...
  methods: {
    ...
    /** 仅获取被 Delete 的数据（新增又被 Delete 的数据不会被获取到） */
    getDeleteData() {
     ...
     * 过滤添加的行
     * @param rows 要筛选的行数据
     * @param remove true =  Delete 新增，false=只 Delete id
    ...
    /**
     * 【 Delete 指定行数据】（重写vxeTable的内部方法，添加了从keepSource中 Delete ）
     * 如果传 row 则 Delete 一行
     * 如果传 rows 则 Delete 多行
     * 如果为空则 Delete 所有
     */
    _remove(rows) {
